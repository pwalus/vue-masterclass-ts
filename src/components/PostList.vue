...
const editModeForPost = ref<string>("");
const userById = (userId: string): User =>
  userStore.users.find((u) => u.id === userId) as User;

const toggleEditMode = (postId: string): void => {
  editModeForPost.value = postId === editModeForPost.value ? "" : postId;
};
</script>
